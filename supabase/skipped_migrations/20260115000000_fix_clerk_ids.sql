...
BEGIN
    ...
    */
    ...
    -- 3.1. DROP POLICIES ON CARTS (Commented out as carts table might not exist yet)
    /*
    BEGIN
    ...
    */
    ...
    -- 4. DROP FOREIGN KEY CONSTRAINTS AND DEFAULTS
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'shops') THEN
        ...
        ALTER TABLE shops ALTER COLUMN user_id DROP DEFAULT; -- Drop default (auth.uid())
        ALTER TABLE shops ALTER COLUMN user_id TYPE text USING user_id::text;
    END IF;

    /*
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'facebook_tokens') THEN
        ALTER TABLE facebook_tokens DROP CONSTRAINT IF EXISTS facebook_tokens_user_id_fkey;
        ALTER TABLE facebook_tokens ALTER COLUMN user_id DROP DEFAULT;
    ...
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'user_facebook_pages') THEN
        ...
        ALTER TABLE user_facebook_pages ALTER COLUMN user_id DROP DEFAULT;
