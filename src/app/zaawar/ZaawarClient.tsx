...
function readVisited(): Set<string> {
    ...
    if (raw !== visitedCacheRaw) {
        visitedCacheRaw = raw;
        try {
            visitedCache = new Set(raw ? (JSON.parse(raw) as string[]) : []);
        } catch {
            visitedCache = EMPTY_VISITED;
        }
    }
    return visitedCache;
}
...
function markVisited(id: string) {
    try {
        const next = new Set(readVisited()).add(id);
        localStorage.setItem(STORAGE_KEY, JSON.stringify([...next]));
        window.dispatchEvent(new Event(VISITED_EVENT));
    } catch {
        /* localStorage хаалттай орчинд явц хадгалагдахгүй */
    }
}
...
export default function ZaawarClient() {
    const topics = useMemo(
        () => [...ZAAWAR_TOPICS].sort((a, b) => a.order - b.order),
        []
    );
    ...
    const activeIndex = active ? topics.indexOf(active) : -1;
    ...
    return (
        <div className="min-h-screen bg-[#06060f] text-white antialiased overflow-x-hidden">
            ...
            <section className="relative z-10 max-w-3xl mx-auto text-center px-4 sm:px-6 pt-6 pb-10">
                ...
                <div className="mt-6 flex items-center justify-center gap-4 flex-wrap">
                    <button
                        ...
                        className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-gradient-to-r from-indigo-500 to-violet-500 text-sm font-semibold hover:opacity-90 transition-opacity shadow-lg shadow-indigo-500/25"
                    >
                    ...
                    <div className="flex items-center gap-2.5 text-xs text-white/50">
                        <div className="w-28 h-1.5 rounded-full bg-white/10 overflow-hidden">
                            <div
                                ...
                                style={{ width: `${(visited.size / topics.length) * 100}%` }}
                            />
                        </div>
                        {visited.size}/{topics.length} сэдэв үзсэн
                    </div>
                </div>
            </section>
            ...
            <section className="relative z-10 hidden md:block max-w-5xl mx-auto px-6 pb-20">
                ...
                >
                    ...
                    >
                        <button
                            onClick={() => openTopic(topics[0].id)}
                            className="relative w-36 h-36 rounded-full bg-gradient-to-b from-indigo-500/30 to-violet-500/10 backdrop-blur ring-1 ring-indigo-400/40 flex flex-col items-center justify-center gap-1.5 hover:scale-105 transition-transform shadow-2xl shadow-indigo-500/20"
                            ...
                            >
                                ...
                                >
                                    <button
                                        ...
                                        className={`relative w-[118px] h-[118px] rounded-full bg-gradient-to-b ${t.color.bubble} backdrop-blur ring-1 ${t.color.ring} flex flex-col items-center justify-center gap-1.5 px-3 hover:scale-110 transition-transform shadow-xl shadow-transparent ${t.color.glow} cursor-pointer`}
                                    >
                                        <span
                                        ...
                                        >
                                            {isVisited ? <Check className="w-3.5 h-3.5" /> : t.order}
                                        </span>
                        ...
                        {topics.map((t) => {
                            ...
                            return (
                                <button
                                    key={t.id}
                                    onClick={() => openTopic(t.id)}
                                    className="relative z-10 w-full flex items-center gap-4 p-4 rounded-2xl bg-[#0c0c1a] ring-1 ring-white/[0.07] hover:ring-white/15 transition-all text-left active:scale-[0.98]"
                                >
                                    <span
                                    ...
                                    >
                                        ...
                                        >
                                            {isVisited ? <Check className="w-3 h-3" /> : t.order}
                                        </span>
                                    </span>
            ...
            <section className="relative z-10 max-w-3xl mx-auto text-center px-4 pb-16">
                ...
                <Link
                    href="/auth/register"
                    className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-white text-[#06060f] text-sm font-semibold hover:bg-white/90 transition-colors"
                >
                    Үнэгүй эхлэх
                ...
                {active && (
                    <>
                        ...
                        <motion.div
                            ...
                            className="fixed z-50 inset-x-0 bottom-0 max-h-[88vh] rounded-t-3xl md:inset-x-auto md:right-0 md:top-0 md:bottom-0 md:max-h-none md:w-[440px] md:rounded-none md:rounded-l-3xl bg-[#0b0b18] ring-1 ring-white/10 flex flex-col"
                            role="dialog"
                            aria-modal="true"
                        ...
                        >
                            ...
                            <div className="flex items-start gap-4 p-5 sm:p-6 border-b border-white/[0.06]">
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className={`text-[11px] font-semibold uppercase tracking-wider ${active.color.text}`}>
                                        Сэдэв {active.order}/{topics.length}
                                    </div>
                                ...
                                <button
                                    onClick={() => setActiveId(null)}
                                    className="shrink-0 w-8 h-8 rounded-full bg-white/[0.06] hover:bg-white/10 flex items-center justify-center transition-colors"
                                    aria-label="Хаах"
                                >
                                    <X className="w-4 h-4" />
                                </button>
                            ...
                            {/* Хөтөчийн навигаци */}
                            <div className="flex items-center justify-between gap-3 p-4 sm:p-5 border-t border-white/[0.06]">
                                <button
                                    onClick={() => openTopic(topics[activeIndex - 1].id)}
                                    disabled={activeIndex <= 0}
                                    className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-sm bg-white/[0.06] ring-1 ring-white/10 hover:bg-white/10 transition-colors disabled:opacity-30 disabled:pointer-events-none"
                                >
                                ...
                                {activeIndex < topics.length - 1 ? (
                                    <button
                                        onClick={() => openTopic(topics[activeIndex + 1].id)}
                                        className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-semibold bg-gradient-to-r from-indigo-500 to-violet-500 hover:opacity-90 transition-opacity"
                                    >
                                        Дараах
                                        <ArrowRight className="w-4 h-4" />
                                    </button>
                                    ...
                                    <button
                                        onClick={() => setActiveId(null)}
                                        className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-semibold bg-gradient-to-r from-emerald-500 to-teal-500 hover:opacity-90 transition-opacity"
                                    >
                                        Дууслаа
            ...
            {/* Bubble-ийн хөвөх хөдөлгөөн + холбоос шугамын урсгал */}
            <style jsx global>{`
                @keyframes zw-float {
                    0%,
                    100% {
                ...
                .zw-dash {
                    animation: zw-dash 4s linear infinite;
                }
