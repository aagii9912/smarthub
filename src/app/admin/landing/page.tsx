        ...
        <div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
            ...
            {open && (
                <div className="border-t border-gray-100 p-4 space-y-4 bg-gray-50/40">
                    ...
                    {/* Slider */}
                    <fieldset className="rounded-lg border border-gray-200 bg-white p-3 space-y-2">
                        <legend className="px-2 text-xs font-bold text-gray-500 uppercase tracking-wider">Slider (visual)</legend>
                        <TogglePill
                            label="Slider идэвхтэй"
                            checked={!!plan.slider}
                            onChange={(v) =>
                                onChange({
                                    ...plan,
                        ...
                        {plan.slider && (
                            <>
                                <div className="grid grid-cols-2 gap-3">
                                    <Field
                                        label="Label"
                                        value={plan.slider.label}
                                        onChange={(v) =>
                                            onChange({ ...plan, slider: { ...plan.slider!, label: v } })
                                        }
                                    />
                                    <Field
                                        label="Утга (хүчтэй)"
                                        value={plan.slider.value}
                                        onChange={(v) =>
                                            onChange({ ...plan, slider: { ...plan.slider!, value: v } })
                                        }
                                    />
                                </div>
                                <div className="grid grid-cols-3 gap-3">
                                    {[0, 1, 2].map((i) => (
                                        <Field
                                            key={i}
                                            label={`Tick ${i + 1}`}
                                            value={plan.slider!.ticks[i]}
                                            onChange={(v) => {
                                                const ticks = [...plan.slider!.ticks] as [string, string, string];
                                                ticks[i] = v;
                                                onChange({ ...plan, slider: { ...plan.slider!, ticks } });
                                            }}
                                        />
                                    ))}
                                </div>
                                <div>
                                    ...
                                    <input
                                        ...
                                        max={100}
                                        className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-900 focus:bg-white focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 transition-all"
                                        value={plan.slider.fillPercent}
                                        onChange={(e) =>
                                            onChange({
                                                ...plan,
                                                slider: { ...plan.slider!, fillPercent: Number(e.target.value) || 0 },
                                            })
                                        }
                                    />
                                </div>
                    ...
                    {/* Seats — only for Business */}
                    {planKey === 'business' && (
                        <fieldset className="rounded-lg border border-gray-200 bg-white p-3 space-y-2">
                            <legend className="px-2 text-xs font-bold text-gray-500 uppercase tracking-wider">
                                Хэрэглэгчийн тоо (seat counter)
                            </legend>
                            <TogglePill
                                label="Seat counter идэвхтэй"
                                checked={!!plan.seats}
                                onChange={(v) =>
                                    onChange({
                                        ...plan,
                                        seats: v
                                            ? plan.seats ?? { enabled: true, defaultCount: 2, label: 'хэрэглэгч' }
                                            : undefined,
                                    })
                                }
                            />
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            max={15}
                                            className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-900 focus:bg-white focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 transition-all"
                                            value={plan.seats.defaultCount}
                                            onChange={(e) =>
                                                onChange({
                                                    ...plan,
                                                    seats: {
                                                        ...plan.seats!,
                                                        defaultCount: Number(e.target.value) || 1,
                                                    },
                                                })
                                            }
                                        />
                                    ...
                                    <Field
                                        ...
                                        value={plan.seats.label}
                                        onChange={(v) =>
                                            onChange({
                                                ...plan,
                                                seats: { ...plan.seats!, label: v },
                                            })
                                        }
                                    />
                                </div>
